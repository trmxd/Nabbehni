...
import { getPracticeSentence } from '../utils/practice';
...
export function WordLens({
  ...
  const practiceSentence = getPracticeSentence(detection.record);
        ...
        >
          ...
          {feedback !== 'correct' ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {practiceSentence || detection.record.examples?.length ? (
                <div className="mt-3">
                  ...
                  {showExamples && (
                    <p className="mt-2 rounded-xl bg-brand-gray-light p-3 text-center text-sm font-bold">
                      {practiceSentence ? <>مثال آخر: {practiceSentence}</> : detection.record.examples?.join('، ')}
                    </p>
                  )}
                </div>
              ) : null}
